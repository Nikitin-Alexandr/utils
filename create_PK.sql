...
select format('create sequence if not exists %I.%I as bigint;',:'schema_name',:'seq_name');
...
select format('alter sequence %I.%I owned by %I.%I;',:'schema_name',:'seq_name',:'tbl_name',:'new_colname');
select '\set QUIET on';
...
select format('create unique index concurrently if not exists %s_%s_idx on %I.%I(%I);',:'tbl_name',:'new_colname',:'schema_name',:'tbl_name',:'new_colname');
select '\set QUIET on';
select $$set statement_timeout to '100ms';$$;
select '\set QUIET off';
